      ...
      <div class="w-full md:w-80 bg-white border-l border-gray-200 flex flex-col z-10 shadow-sm">
        ...
        <div class="flex-1 overflow-y-auto p-4">
                ...
                @for (clip of selectedPageClips(); track clip.id) {
                  <div class="bg-white border border-gray-200 rounded-lg p-3 hover:shadow-md transition group">
                    ...
                    <div class="flex items-center justify-between mt-2">
                       <span class="text-xs text-gray-500">Article Linked: {{ clip.article_id ? 'Yes' : 'No' }}</span>
                    </div>
                  </div>
                }
              </div>
          ...
          @if (activeTab === 'properties') {
            @if (selectedPage()) {
              <form [formGroup]="clipForm" class="space-y-4">
                ...
                <p class="text-sm text-gray-500">Page Number: {{ selectedPage()!.page_number }}</p>
                <p class="text-sm text-gray-500">ID: {{ selectedPage()!.id }}</p>
              </form>
            }
          }
        </div>
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
          ...
          <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Create New Clip</h3>
              <div class="mt-4 space-y-4">
                <div class="h-32 bg-gray-100 rounded border border-gray-200 overflow-hidden flex justify-center">
                  <img [src]="tempClipImage" class="h-full object-contain">
                </div>
                ...
                <form [formGroup]="clipForm">
                  <div class="space-y-4">
                    <div>
                      ...
                      <input formControlName="headline" type="text" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>
                    <div>
                      <label class="block text-sm font-medium text-gray-700">Link to Article (Optional)</label>
                      <select formControlName="article_id" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
            ...
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
              ...
              <button (click)="cancelClip()" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                Cancel
              </button>
            </div>
          </div>
...
export class EPaperDashboardComponent implements OnInit {
  ...
  prepareClip() {
    ...
    if (canvas) {
      this.tempClipImage = canvas.toDataURL('image/jpeg');
      this.clipForm.reset();
